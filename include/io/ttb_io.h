...
/* Load a TTBL file into a newly allocated in-memory table. */
...
/* Return non-zero when the path looks like a legacy directory-based book. */
...
/* Load the active table from a legacy TTBX directory book. */
Table *ttbx_load(const char *path, char *err, size_t err_sz);
/* Load a specific table entry from a legacy TTBX directory book. */
...
/* Save the active in-memory table as the primary table in a legacy book. */
int ttbx_save(const Table *table, const char *path, char *err, size_t err_sz);
/* Save or replace one table entry inside a legacy directory-based book. */
